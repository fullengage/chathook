...
const EntradaPage: React.FC<EntradaPageProps> = () => {
  ...
  const loadContatos = async (pagina: number = 1, query?: string) => {
    console.log(`🔄 Carregando contatos - Página ${pagina}${query ? ` - Busca: "${query}"` : ''}`);
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (query && query.trim()) {
        result = await searchContacts(query.trim());
      } else {
        result = await getContacts(pagina, 25, 'last_activity_at');
      }
  ...
  const handleBusca = async (query: string) => {
    setBusca(query);
    if (query.trim()) {
      await loadContatos(1, query);
    } else {
      await loadContatos(1);
    }
  };
      ...
      <aside className="w-80 border-r flex flex-col">
        ...
        <div className="p-4 border-b bg-gray-50">
          <div className="flex justify-between items-center mb-3">
            <h2 className="font-semibold">
              Entrada ({meta?.total_count || contatos.length})
            </h2>
        ...
        {meta && meta.total_pages > 1 && (
          <div className="p-3 border-t bg-gray-50 flex justify-between items-center">
            <button
              onClick={() => loadContatos(paginaAtual - 1, busca)}
            ...
            <div className="p-4 border-b bg-gray-50">
              <div className="flex items-center">
                ...
                <div className="flex-1">
                  <h1 className="text-xl font-bold">{contatoAtivo.name}</h1>
